...
class MapperApp:
    ...
    def setup_ui(self):
        ...
        self.left_sidebar = tk.Frame(
            self.root, width=200, bg="#1e293b", padx=10, pady=10)
        ...
        tk.Label(
            ...
            font=(
                ...
                "bold")).pack(
            pady=5)
        self.btn_new_floor = tk.Button(
            self.left_sidebar,
            text="+ Add Floor",
            command=self.add_floor_wizard)
        ...
        self.btn_upload_logo = tk.Button(
            self.left_sidebar,
        ...
        self.btn_sprite = tk.Button(
            ...
            font=(
                ...
                10,
                "bold"),
            command=self.generate_sprite_sheet)
        ...
        self.canvas_label = tk.Label(
            self.center_frame,
            text="Select a floor from the left menu",
            font=(
                "Arial",
                14),
            bg="#f1f5f9",
    ...
    def upload_logo(self):
        source_path = filedialog.askopenfilename(
            title="Select Logo Image", filetypes=[
                ("Images", "*.png *.jpg *.jpeg")], parent=self.root)
        if not source_path:
            return
        ...
        store_name = simpledialog.askstring(
            "Store Name",
            "Which store is this logo for?\n(Type it exactly as it appears on the map)",
            parent=self.root)
        if not store_name:
            return
        ...
        try:
            ...
            messagebox.showinfo(
                "Success",
                f"Logo added to Library as '{store_name}{ext}'!",
                parent=self.root)
        except Exception as e:
            messagebox.showerror(
                "Error",
                f"Failed to copy image: {e}",
                parent=self.root)
    ...
    def add_floor_wizard(self):
        folder_name = simpledialog.askstring(
            "New Floor",
            "Enter Folder Number/ID (e.g., 0, 1, B1):",
            parent=self.root)
        if not folder_name:
            return
        ...
        display_name = simpledialog.askstring(
            "New Floor",
            f"Enter Display Name for '{folder_name}':",
            parent=self.root)
        if not display_name:
            return
        ...
        messagebox.showinfo(
            "Map Image",
            "Please select the Map Image (.png or .jpg) for this floor.",
            parent=self.root)
        image_source = filedialog.askopenfilename(
            title="Select Map Image", filetypes=[
                ("Images", "*.png *.jpg")], parent=self.root)
        if not image_source:
            return
        ...
        if not any(f['folder'] == folder_name for f in self.floor_config):
            ...
            messagebox.showinfo(
                "Success",
                f"Floor '{display_name}' added to local staging!",
                parent=self.root)
    ...
    def export_project(self):
        ...
        messagebox.showinfo(
            "Export",
            "Select the 'public' folder of your React website project.\n\n(e.g., interactive-floor-plan/public)")
        target_web_dir = filedialog.askdirectory(
    ...
    def on_canvas_click(self, event):
        if not self.active_floor_folder:
            return
    ...
    def delete_store(self):
        ...
        if messagebox.askyesno("Delete",
            ...
            self.stores = [
                s for s in self.stores if s['id'] != self.selected_store_id]
            self.redraw_canvas()
            self.reset_editor()
