...
export default function InfoPanel({ store, onClose }) {
  if (!store) {
    return (
      <div className="empty-state">
        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ width: 48, opacity: 0.3, marginBottom: 15 }}>
  ...
  return (
    <div className="store-details">
      <button 
        onClick={onClose}
        style={{ float: 'right', border: 'none', background: 'none', cursor: 'pointer', fontSize: '24px', color: '#94a3b8' }}
      >
        ×
      </button>
      ...
      <div style={{ marginBottom: '32px' }}>
        <StoreIcon spriteX={store.spriteX} spriteY={store.spriteY} name={store.name} />
      </div>
